...
The cross-cache floor pass runs only under the machine lock, so it is reached only when that lock
exists: in the run, and in the `commit` of a write that finds the volume past the floor. A root
that is named but absent is as dark as an unset one here, exactly as it is everywhere else.
...
def commit(name, path, reason=None):
    ...
    The floor pass here runs only under the machine lock, like the run's: it evicts across caches
    by the ranking, and the lock this call holds is one cache's. Without that lock (no root, or a
    root that is named but absent, which is as dark as no root) there is nothing to serialise the
    other caches' deletions against, so `reserve`'s refusal past the floor and the next run's age
    and cap passes are what bounds the machine instead."""
    rec = record(name)
    release(name, path)
    if rec is None:
        return False, "unregistered"
    ...
    # `lock_path()` with no name IS the machine lock, and it is None unless the root is on disk:
    # the same condition, asked in one place, so a root that is named but absent counts as no
    # machine lock rather than as a root.
    past_floor = lock_path() is not None and over_floor(root)
